...
export default function Edit({ phone, globalLineData, phoneButtonTemplate, mohAudioSources, screenCaptures }: Props) {
    ...
    useEffect(() => {
        if (data.ucm_cluster_id && (phone as any).uuid) {
            loadExtensionMobilityData();
        }
    }, [data.ucm_cluster_id, (phone as any).uuid]);
    ...
    const loadDevicePools = async () => {
        if (devicePools.length === 0) {
            try {
                const response = await fetch(`/api/ucm/${data.ucm_cluster_id}/options/device-pools`);
    ...
    const loadPhoneModels = async () => {
        if (phoneModels.length === 0) {
            try {
                const response = await fetch(`/api/ucm/${data.ucm_cluster_id}/options/phone-models`);
                const responseData = await response.json();
    ...
    const loadCommonDeviceConfigs = async () => {
        if (commonDeviceConfigs.length === 0) {
            try {
                const response = await fetch(`/api/ucm/${data.ucm_cluster_id}/options/common-device-configs`);
                const responseData = await response.json();
    ...
    const loadPhoneButtonTemplates = async () => {
        if (phoneButtonTemplates.length === 0) {
            try {
                ...
                const url = `/api/ucm/${data.ucm_cluster_id}/options/phone-button-templates${params.toString() ? '?' + params.toString() : ''}`;
    ...
    const loadCommonPhoneConfigs = async () => {
        if (commonPhoneConfigs.length === 0) {
            try {
                const response = await fetch(`/api/ucm/${data.ucm_cluster_id}/options/common-phone-configs`);
                const responseData = await response.json();
    ...
    const loadCallingSearchSpaces = async () => {
        if (callingSearchSpaces.length === 0) {
            try {
                const response = await fetch(`/api/ucm/${data.ucm_cluster_id}/options/calling-search-spaces`);
                const responseData = await response.json();
    ...
    const loadLocations = async () => {
        if (locations.length === 0) {
            try {
                const response = await fetch(`/api/ucm/${data.ucm_cluster_id}/options/locations`);
                const responseData = await response.json();
    ...
    const loadMediaResourceGroupLists = async () => {
        if (mediaResourceGroupLists.length === 0) {
            try {
                const response = await fetch(`/api/ucm/${data.ucm_cluster_id}/options/media-resource-group-lists`);
                const responseData = await response.json();
    ...
    const loadAarGroups = async () => {
        if (aarGroups.length === 0) {
            try {
                const response = await fetch(`/api/ucm/${data.ucm_cluster_id}/options/aar-groups`);
                const responseData = await response.json();
    ...
    const loadUserLocales = async () => {
        if (userLocales.length === 0) {
            try {
                const response = await fetch(`/api/ucm/${data.ucm_cluster_id}/options/user-locales`);
                const responseData = await response.json();
    ...
    const loadGeoLocations = async () => {
        if (geoLocations.length === 0) {
            try {
                const response = await fetch(`/api/ucm/${data.ucm_cluster_id}/options/geo-locations`);
                const responseData = await response.json();
    ...
    const loadUcmUsers = async () => {
        if (ucmUsers.length === 0) {
            try {
                const response = await fetch(`/api/ucm/${data.ucm_cluster_id}/options/ucm-users`);
                const responseData = await response.json();
    ...
    const loadMobilityUsers = async () => {
        if (mobilityUsers.length === 0) {
            try {
                const response = await fetch(`/api/ucm/${data.ucm_cluster_id}/options/mobility-users`);
                const responseData = await response.json();
    ...
    const loadPhones = async () => {
        if (phones.length === 0) {
            try {
                const response = await fetch(`/api/ucm/${data.ucm_cluster_id}/options/phones`);
                const responseData = await response.json();
    ...
    const loadPresenceGroups = async () => {
        if (presenceGroups.length === 0) {
            try {
                const response = await fetch(`/api/ucm/${data.ucm_cluster_id}/options/presence-groups`);
                const responseData = await response.json();
    ...
    const loadSipDialRules = async () => {
        if (sipDialRules.length === 0) {
            try {
                const response = await fetch(`/api/ucm/${data.ucm_cluster_id}/options/sip-dial-rules`);
                const responseData = await response.json();
    ...
    const loadPhoneSecurityProfiles = async () => {
        if (phoneSecurityProfiles.length === 0) {
            try {
                ...
                const url = `/api/ucm/${data.ucm_cluster_id}/options/phone-security-profiles${params.toString() ? '?' + params.toString() : ''}`;
    ...
    const loadSipProfiles = async () => {
        if (sipProfiles.length === 0) {
            try {
                const response = await fetch(`/api/ucm/${data.ucm_cluster_id}/options/sip-profiles`);
                const responseData = await response.json();
    ...
    const loadDeviceProfiles = async () => {
        if (deviceProfiles.length === 0) {
            try {
                ...
                const url = `/api/ucm/${data.ucm_cluster_id}/options/device-profiles${params.toString() ? '?' + params.toString() : ''}`;
        ...
        if (!extensionMobilityLoading) {
            ...
            try {
                ...
                if (phoneId) {
                    ...
                    const url = `/api/ucm/${data.ucm_cluster_id}/options/extension-mobility-dynamic?${params.toString()}`;
